...
A value that is either `text`, `sort`, or `search`. Any other values will cause the control to render as a Text Header.

###### `DynamicSortBinding` Interface

Specifies bindings to your application that will be triggered when changes to the Dynamic Header's sort state is altered. Due to the scope of `this`, it is not recommended to directly pass a reference to a function. Instead wrap that function in an anonymous JavaScript arrow function.
...
let mySearchColumn = {
	...
	// ...
	sort: {
		activeSortDirectionChange: sortDirection => (this.activeSortDirection = sortDirection),
		activeSortKeyChange: sortKey => (this.activeSortKey = sortKey),
		sortChange: sortTuple => {}
	}
};
```

####### `activeSortDirectionChange: (sortDirection: string) => void` Callback
Fires when the Sort Direction (asc or desc) is changed by a control within a Dynamic Header.
	...
	columns: DynamicColumn[] = [
		{
			...
			// ...
			sort: {
				activeSortDirectionChange: sortDirection => {
...
####### `activeSortKeyChange: (sortKey: string) => void` Callback
Fires when the Sort Key is changed by a control within a Dynamic Header.
...
####### `sortChange: (sortTuple: SortTuple) => void` Callback
Fires when either the Sort Key or Sort Direction is changed by a control within a Dynamic Header. Typically, this is the fastest way to update search state within your application. Since it consolidates Sort Key and Sort Direction changes into one call, it is recommended to bind to this callback if the action triggers an expensive calculation, such as querying a server.
	...
	columns: DynamicColumn[] = [
		{
			...
			// ...
			sort: {
				sortChange: (sortTuple: SortTuple) => {
					let [sortKey, sortDirection] = sortTuple;
					this.activeSortKey = sortKey;
					this.activeSortDirection = sortDirection;
				}
			}
...
###### `DynamicSearchBinding` Interface

TBD

###### `DynamicHeader` Interface.
...
This defines the behavior and presentation of the column. When set to `search`, the `DynamicColumn` should contain a `DynamicSearchBinding` at the `search` property.
...
This is the user visible label for the header.

####### `key: string`

This is an identifier that will be used to uniquely identify the columns in the current list.

####### `locked: boolean`

Used to lock a column so that it cannot be removed from the current view. This does not affect the Dynamic Header presentation, but it can be used with components like the Rack.

####### `width: number`

Specifies the width of the column. These are expressed as percentages (a float between 0 and 1), and are most easily managed by using fractions.

```typescript
let myColumn = {
	width: 75 / 100 // use fractions to make sizes easy to manage
};
```

####### `sort?: DynamicSortBinding`

Contains metadata used to bind the result of any search columns

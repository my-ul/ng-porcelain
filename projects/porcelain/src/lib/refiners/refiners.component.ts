	...
	toQueryString(): string {
		return Object.keys(this.values)
			.reduce((result, paramKey) => {
				if (
					// None are selected, return result so far
					Array.isArray(this.values[paramKey]) &&
					this.values[paramKey].length === 0
				) {
					return result;
				} else if (
					// Some are selected, return result + new options
					Array.isArray(this.values[paramKey]) &&
					this.values[paramKey].length > 0
				) {
					return [
						...
						encodeURIComponent(paramKey) +
							'=' +
							this.values[paramKey].map(val => encodeURIComponent(val)).join(',')
					];
				} else if (
					// Is an object, like a dictionary (start, end date)
					...
					return [
						...
						Object.keys(this.values[paramKey])
							.map(nestedKey => {
								let valueToEncode =
									this.values[paramKey][nestedKey] instanceof Date
										? this.values[paramKey][nestedKey].toJSON()
										: this.values[paramKey][nestedKey];
								return (
									`${encodeURIComponent(paramKey)}[${encodeURIComponent(nestedKey)}]` +
									'=' +

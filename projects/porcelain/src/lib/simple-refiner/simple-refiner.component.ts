...
export class SimpleRefinerComponent implements OnInit {
	...
	ngOnInit() {
		...
		if (this.refiner.options) {
			for (let optionSlug in this.refiner.options) {
				...
				if (option instanceof SimpleOption || option.hasOwnProperty('isSelected')) {
	...
	getOptionBadge(option: SimpleOption) {
		if (typeof option.badge === 'number') return option.badge.toLocaleString();
	...
	canSelectNone(): boolean {
		return Object.keys(this.value).every(paramName => this.value[paramName] === true);
	}
